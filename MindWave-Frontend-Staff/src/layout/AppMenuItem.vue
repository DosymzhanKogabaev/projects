...
const iconClass = computed(() => {
    return {
        'pi-angle-up': isOpen.value === true,
        'pi-angle-down':  isOpen.value === false
    };
});
...
const itemClick = async (event, item) => {
    ...
    if ((item.link || item.url) && (staticMenuMobileActive.value || overlayMenuActive.value)) {
        onMenuToggle();
    }
...
<template>
    <li ref="menuItemRef" :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
        <div v-if="root && item.visible !== false" class="layout-menuitem-root-text" @click="openMenu">
            <span>{{ item.title }}</span> <i class="pi pi-fw" :class="iconClass"></i>
        </div>
        <a v-if="(!item.link || item.subItem) && item.visible !== false" :href="item.link" @click="itemClick($event, item, index)" tabindex="0" @mouseenter="onMouseEnter" v-tooltip.hover="isSlim && root && !isActiveMenu ? item.title : null">
            <i class="pi pi-home layout-menuitem-icon"></i>
            <span class="layout-menuitem-text">{{ item.title }}</span>
            <i class="pi pi-fw pi-angle-down layout-submenu-toggler" v-if="item.sublink"></i>
        </a>
        <router-link
            v-if="item.link && !item.subItem && item.visible !== false && item.table_id!=103"
            @click="itemClick($event, item, index)"
            :class="[item.class, { 'active-route': checkActiveRoute(item) }]"
            tabindex="0"
            :to="{ name: 'dynamic-table', params: { table: item.table_id } }"
            @mouseenter="onMouseEnter"
            v-tooltip.hover="(isSlim || isSlimPlus) && root && !isActiveMenu ? item.title : null"
        >
            <i class="pi pi-fw pi-exclamation-circle layout-menuitem-icon"></i>
            <span class="layout-menuitem-text">{{ item.title }} {{ item.table_id }}</span>
            <i class="pi pi-fw pi-angle-down layout-submenu-toggler" v-if="item.subItem"></i>
        </router-link>
        <router-link
            v-else-if="item.link && !item.subItem && item.visible !== false && item.table_id==103"
            @click="itemClick($event, item, index)"
            :class="[item.class, { 'active-route': checkActiveRoute(item) }]"
            tabindex="0"
            :to="{ name: 'categories' }"
        ...
        <ul ref="subMenuRef" :class="{ 'layout-root-submenulist': root }" v-if="item.subItem && item.visible !== false && isOpen">
            <AppMenuItem v-for="(child, i) in item.subItem" :key="child" :index="i" :item="child" :parentItemKey="itemKey" :root="false"></AppMenuItem>
        </ul>
    </li>
</template>
